    ...
    def train(self, train, dev):
        ...
        with tf.Session(config=self.config) as sess:
            ...
            for epoch in range(self.epoch_num):
                self.run_one_epoch(
                    sess, train, dev, self.tag2label, epoch, saver)
...
#################################################################################################
    ...
    def evaluate(self, label_list, seq_len_list, data, epoch=None):
        ...
        label_real, label_predict = list(), list()

        for label_, (sent, tag) in zip(label_list, data):
            for i in range(len(sent)):
                # 词，预测标签，真实标签
                label_predict.append(label_[i])
                label_real.append(self.tag2label[tag[i]])

        # sklearn.metrics.classification_report(y_true,y_pred,labels=None,target_names=None,sample_weight=None,digits=2)
        # 准确率就是找得对，召回率就是找得全。
        target_names = ["O", "B-PER", "I-PER",
                        "B-LOC", "I-LOC", "B-ORG", "I-ORG"]
        print(classification_report(label_real,
                                    label_predict,
    ...
    def dev_one_epoch(self, sess, dev):
        ...
        :return:
        """
        label_list, seq_len_list = list(), list()
